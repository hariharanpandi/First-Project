...
export default class AzureImage {
    ...
    async saveImage(imgFile: Record<string, any>, blobName: any) {
        try {
            ...
            logger.info(appConstant.LOG_MESSAGES.SAVE_IMG);
            return blobUrl;
        } catch (error: any) {
            logger.error(`${appConstant.LOG_MESSAGES.IMG_FAILED} ${error.message}`);
    ...
    async deleteImage(appData: any) {
        try {
            ...
            if (!exists) {
                logger.error(appConstant.LOG_MESSAGES.BLOB_NOT_EXIST);
                return; // Or throw an error if needed
            }
            await blobClient.delete();
            logger.info(appConstant.LOG_MESSAGES.BLOB_DELETED)
