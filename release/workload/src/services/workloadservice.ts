...
import { WorkloadUser, findAndUpdateRole, findAndUpdateWorkloadNewUser, findAndUpdateWorkloadUser, findByWorkloadUserMapFields, isUserExist, removeAndupdateWorkload, workloadUserCreate } from '../models/workloadusermapping';
...
export default class WorkloadService {
                ...
                for (const app of appdtl) {
                    if (role_id != undefined) {
                        if (appConstant.ROLES.includes(role_name) || app.workloaddtl===undefined) {
                            ...
                            const workloads = await findAllWorkload(conditions);
                            workloads.map((ele) => {
                    ...
                    for (const workload of workloaddtl) {
                        ...
                        const workload_role_name = workload.role_name != undefined ? appConstant.WORKLOAD_ADMIN : project.role_name;
                        if (!isActive) {
                            await new WorkloadService().removeUserWorkload(application_id, user_id, workload_id);
                            if (project.isActive) {
                                if (appConstant.ROLES[1] === project.role_name) {
                                    const dynamicFields = {
                                        user_id,
                                        role_id: project.role_id ? project.role_id : project.infra_role_id,
                                        role_name: project.role_name
                                    }
                                    await findAndUpdateWorkloadUser(application_id, workload_id, dynamicFields);
                                }
                                const workloadIsExists: any = await findByWorkloadUserMapFields({ project_id, 'workload.user_id': user_id });
                                if (workloadIsExists.length == 0) {
                                    projectIsActive.push({ project_id })
                                }
                            }
                        } else {
                            // const project_app_map_id: string = project_app_map_ids[appIndex];
                            const isAppAndWorkloadExist: any = await findByWorkloadUserMapFields({ application_id, workload_id, status: appConstant.SCHEMA.STATUS_ACTIVE });
                            if (!_.isNil(isAppAndWorkloadExist) && isAppAndWorkloadExist.length > 0) {
                                const isUserExist: any = await findByWorkloadUserMapFields({ application_id, workload_id, "workload.user_id": user_id });
                                if ((!_.isNil(isUserExist) && isUserExist.length > 0)) {
                                    // const isAppAndWorkloadExist = isUserExist && isUserExist.find((ele: any) => { return ele.workload_id === workload_id; });
                                    // if ((project.role_name === appConstant.ROLES[1] || project.role_name === appConstant.WORKLOAD_ADMIN) && ) {
                                    const updateUserRole = await findAndUpdateRole(user_id, workload_id, workload_role_id, workload_role_name);
                                    // }
                                } else {
                                    const dynamicFields = {
                                        user_id,
                                    ...
                                    const updatedRecord = await findAndUpdateWorkloadUser(application_id, workload_id, dynamicFields)
                                }
                            } else {
                                const appUserMap = {
                                ...
                                const appRes: any = await workloadUserCreate(appUserMap);
                            }
                        }
                    }
                    appIndex++;
     ...
     */
    async removeUserWorkload(application_id: string, user_id: string, workload_id: string,): Promise<any> {
        try {
            await removeAndupdateWorkload(application_id, user_id, workload_id);
        } catch (error: any) {
            logger.error(appConstant.LOGGER_MESSAGE.REMOVE_USER_WORKLOAD_FAILED)
            throw new Error(error.message)
        }
    }
