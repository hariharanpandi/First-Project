...
import { applicationUserCreate, findByApplicationUserMapFields, findAndUpdateApplicationUser, findAndUpdateAppNewUser, ApplicationUser, removeAndupdateApplication, isUserExist, getAppUsers } from "../models/userapplicationmapping";
const logger = require('../helpers/logger');
import DateConvertor from "../helpers/date";
import { CloudOnbarod, cloudOnboardCreate, cloudUserCount, cloundAccountUpdate, findAll, findAllById, findCloudAccountAndUpdate, findCloudAccountByFields } from "../models/cloudonboard";
...
export default class ProjectService {
    ...
    */
    ...
    async projectUserMapUpdate(projectData: Record<string, any>, tokenData: Record<string, any>, query: Record<string, any>, req: any) {
        try {
            ...
            for (let { project_id, role_id, isActive, role_name } of projectdtl) {
                ...
                if (!isActive) {
                    await new ProjectService().removeByProjectUserMap(project_id, user_id);
                    ...
                    if (role_id != undefined) {
                        ...
                        if (project) {
                            const isUserExist = await findByProjectUserMapFields({ project_id, "users_mapping.user_id": user_id });
                            if (!isUserExist) {
                                const projectUserMapRes = await findAndUpdateProjectUser(project_id, projectUserMap);
                                project_user_map_ids.push(projectUserMapRes?._id);
                                ...
                                const updateRole = await findAndUpdateRoleProjectUser(updateProjectUser, project_id);
                                auditTrailActivity.push({ project_id, project_name, activity: appConstant.AUDIT_TRAIL.PROJECT_MAP_UPDATED });
                            }
                        } else {
                            ...
                            const projectUserMapRes = await projectUserCreate(projectUserMapData);
                            project_user_map_ids.push(projectUserMapRes?._id);
            ...
            const applicationUserMapIds = await self.applicationUserMapCreate(projectData, tokenData, project_user_map_ids);
            logger.info(appConstant.LOG_MESSAGES.USER_MAP_WORKLOAD_CALLED);
            await otherService.workloadUserMap(req, projectData, applicationUserMapIds).then(async (response) => {
                if(response?.error){
     ...
     */
    async applicationUserMapCreate(projectData: Record<string, any>, tokenData: Record<string, any>, project_user_map_ids: any) {
        try {
            const { user_id, projectdtl } = projectData;
            const project_app_map_id: any[] = [];
            let projectIndex = 0;
                ...
                if (role_id != undefined) {
                    if (appConstant.ROLES.includes(role_name) || appdtl.length === 0) {
                        ...
                        applications.map((ele) => {
                            if (!application_ids.includes(ele._id.toString())) {
                                project.appdtl.push({ application_id: ele._id, isActive: isActive })
                            }
                        })
                    }
                    for (const app of appdtl) {
                        ...
                        if (!isActive) {
                            await new ProjectService().removeByAppUserMap(project_id, application_id, user_id)
                        } else {
                            const project_user_map_id: string = project_user_map_ids[projectIndex];
                            const project = await findByApplicationUserMapFields({ project_id, status: appConstant.SCHEMA.STATUS_ACTIVE });
                            if (project) {
                                const applicationExists = await findByApplicationUserMapFields({ project_id, "application.application_id": application_id, status: appConstant.SCHEMA.STATUS_ACTIVE });
                                if (applicationExists) {
                                    const isUserExists = await isUserExist({ project_id, application_id, user_id });
                                    if (!isUserExists) {
                                        const appRes = await findAndUpdateApplicationUser(project_id, application_id, user_id);
                                        project_app_map_id.push(appRes?._id);
                                    }
                                } else {
                                    const appUserMap = {
                                        user_id: [user_id],
                                    ...
                                    const appRes = await findAndUpdateAppNewUser(project_id, appUserMap);
                                    project_app_map_id.push(appRes?._id);
                                }
                            } else {
                                const appUserMap = {
    ...
    /**
     * This function is used to remove the mapped projects with user
     */
    async removeByProjectUserMap(project_id: string, user_id: string): Promise<any> {
        try {
            const removeUserMappedProject = await removeAndupdate(project_id, user_id);
     ...
     * This function is used to remove the mapped projects with user
     */
    async removeByAppUserMap(project_id: string, application_id: string, user_id: string): Promise<any> {
        try {
            const removeUserMappedProject = await removeAndupdateApplication(project_id, user_id, application_id);
        } catch (error: any) {
            logger.error(`${appConstant.LOG_MESSAGES.USER_MAP_FAILED} ${error.message}`);
            throw new Error(error.message)
        }
